...
rules:
  ...
  - id: working_artifacts_review
    ...
    severity: hard
  - id: working_artifacts_lessons
    statement: Record execution errors, root causes, and fixes in lessons.md (consolidated automatically via /dream and idle loops).
    severity: hard
  ...
  - id: session_storage_organization
    statement: Keep each session's task.md, plan.md, lessons.md, and session.json inside a homonymous folder inside sessions/.
    severity: hard
...
4. Post-action reflection: Record execution errors, root causes, and learned fixes in `lessons.md` (or run `/dream` / `/lessons`) to prevent repeating mistakes.
5. Long logs/transcripts: use `/attach <path>` or `@\"path\"` — never paste huge terminal dumps.

## Workspace

6. Home is the process cwd (Tiny Steward repo root). Prefer relative paths from that home.
7. Wingman Agent: support, teach, and keep child Tiny-Steward involved in delegated tasks. Size strengths, motivate progress, and share session experiences.

## Session Storage Organization

- Each `session.json` should be inside a homonymous folder (to stop polluting `sessions/` folder).
- Keep each session's `task.md`, `plan.md`, and `lessons.md` files inside that same folder.
